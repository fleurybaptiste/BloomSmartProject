...
// Utiliser Serial.print pour afficher les lectures sur le moniteur série
if (isnan(humidity) || isnan(temperature)) {
Serial.println(F("Failed to read from DHT sensor!"));
} else {
// Afficher les lectures sur le moniteur série
...
// Afficher les lectures sur l'écran OLED
display.clearDisplay();
...
if (WiFi.status() == WL_CONNECTED) {
    HTTPClient http;
    http.begin(client, serverUrl); // Commencez la connexion avec le client WiFi

    http.addHeader("Content-Type", "application/json");

    // Créez l'objet JSON pour l'envoi
    ...
    http.end(); // Terminez la connexion
}
}